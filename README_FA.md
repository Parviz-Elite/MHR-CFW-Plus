...
ابتدا صفحه اصلی کنترل محلی را باز کنید:

```text
http://127.0.0.1:8085/
```

این صفحه به دو بخش اصلی لینک می‌دهد:
...
برای دیدن وضعیت زنده مسیرها و کنترل‌های امن runtime از `/status` استفاده کنید:
...
برای دیدن و ویرایش تنظیمات از `/dashboard` استفاده کنید. مقدارهای حساس مثل `auth_key` به صورت `***` نمایش داده می‌شوند؛ اگر آن‌ها را تغییر ندهید، مقدار قبلی حفظ می‌شود. تغییرات ذخیره‌شده در `config.json` نوشته می‌شوند و برای اعمال کامل باید پروکسی را ری‌استارت کنید.
